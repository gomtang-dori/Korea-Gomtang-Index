#!/usr/bin/env python3
"""
PyKRX fundamentals 안정 강화형 수집

추가(중요)
- FUND_RAW_DIR env 지원: 출력 폴더 변경 가능 (matrix 연도별 저장용)

env
- START_DATE, END_DATE
- MAX_WORKERS_FUNDAMENTALS
- FUND_SAVE_FORMAT (parquet|csv)
- FUND_RETRY, FUND_EMPTY_RETRY, FUND_EMPTY_SLEEP_BASE
- FUND_NO_DATA_RETRY, FUND_NO_DATA_SLEEP_BASE
- FUND_THROTTLE_SEC
- FUND_APPEND_IF_EXISTS
- HEARTBEAT_SEC
- PROGRESS_JSON
- FUND_RAW_DIR (default data/stocks/raw/fundamentals)
"""
...
FUND_RAW_DIR = Path(os.getenv("FUND_RAW_DIR", str(PROJECT_ROOT / "data/stocks/raw/fundamentals")))
...
def fetch_one_ticker(ticker: str) -> tuple[str, str]:
    ...
    try:
        ...
        if FUND_APPEND_IF_EXISTS and (out_base.with_suffix(".parquet").exists() or out_base.with_suffix(".csv").exists()):
            old = _load_existing(out_base)
            if not old.empty:
...
def main():
    ...
    print(f"  out_dir={FUND_RAW_DIR}")
    print(f"  workers={MAX_WORKERS_FUND}, throttle={FUND_THROTTLE_SEC}s, retry={RETRY}, empty_retry={EMPTY_RETRY}, nodata_retry={NO_DATA_RETRY}, append={FUND_APPEND_IF_EXISTS}")
