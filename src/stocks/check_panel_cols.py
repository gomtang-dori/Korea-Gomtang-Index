...
def main():
    ...
    req = {
        ...
        "SIZE": ["market_cap", "shares", "value"],
        "WICS": ["wics_major", "wics_mid"],
    }
    ...
    print(f"[panel-check] rows={pf.metadata.num_rows:,} cols={len(schema):,}")
    print(f"[panel-check] dart_required={dart_required} (dart_std={dart_std})")
    ...
    for g, cols in req.items():
        ...
        for c in cols:
            ...
            print(f"- {c}: OK  nonnull={nn:,} ({pct:.1f}%)  all_nan={allna}")
