...
def _get_all_items(endpoint: str, service_key: str, begin: str, end: str, sleep_s: float = 0.1) -> list[dict]:
    """
    data.go.kr 표준 응답: response > body > items > item
    resultType=json 사용 (실패 시 에러 본문 출력)
    """
    url = f"{BASE}/{endpoint}"
    items: list[dict] = []
    ...
    while True:
        ...
        if r.status_code != 200:
            raise RuntimeError(
                f"[f09_cache] HTTP {r.status_code} {endpoint} "
        ...
        try:
            ...
            raise RuntimeError(f"[f09_cache] unexpected json shape endpoint={endpoint} keys={list(data.keys())}")
...
def main():
    ...
    if credit.empty:
        raise RuntimeError("[f09_cache] credit empty. Check serviceKey / 승인 / 파라미터.")
    if "basDt" not in credit.columns or "crdTrFingWhl" not in credit.columns:
        raise RuntimeError(f"[f09_cache] credit missing cols. cols={list(credit.columns)}")
    ...
    # 2) Deposit proxy (증시자금추이)
    cap_items = _get_all_items(
        endpoint="getSecuritiesMarketTotalCapitalInfo",
        service_key=service_key,
        begin=begin_s,
        end=end_s,
    )
    dep = pd.DataFrame(cap_items)
    if dep.empty:
        raise RuntimeError("[f09_cache] deposit empty. Check serviceKey / 승인 / 파라미터.")
    if "basDt" not in dep.columns or "invrDpsgAmt" not in dep.columns:
        raise RuntimeError(f"[f09_cache] deposit missing cols. cols={list(dep.columns)}")
    ...
    merged = merged.replace([pd.NA, pd.NaT, float("inf"), float("-inf")], pd.NA).dropna(subset=["margin_ratio_raw"])
    ...
    if out_path.exists():
        ...
        merged2 = pd.concat([old, merged], ignore_index=True)
        merged2 = merged2.drop_duplicates(subset=["date"], keep="last").sort_values("date").reset_index(drop=True)
    else:
        merged2 = merged

    out_path.parent.mkdir(parents=True, exist_ok=True)
    merged2.to_parquet(out_path, index=False)
    print(f"[f09_cache] OK rows={len(merged2)} -> {out_path}")
